...
async function getSpaceWeather(){
	...
	const [kpResult,magResult,kpfResult]=await Promise.allSettled([
...
const w=clamp(1-hoursAhead/3,0,1);return w*ovation+(1-w)*kpActivity(mlat,kp)}
const activityLabel=a=>a<10?'Niska':a<35?'Umiarkowana':'Wysoka';
function darkLabel(sunAlt){return sunAlt>=-6?'Nie':sunAlt>=-12?'Zmierzch':sunAlt>=-18?'Prawie pełna':'Pełna'}
