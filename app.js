...
function renderSpaceOnly(loc,grid,space){const act=auroraAt(loc.lat,loc.lon,grid)??kpVisible(magLat(loc.lat,loc.lon),space.kp??2);els.activity.textContent=activityLabel(act);els.activitySub.textContent=[space.kp!=null?`Kp ${space.kp.toFixed(1)}`:'',space.bzAvg!=null?`Bz śr. ${fmtNum(space.bzAvg)} nT`:''].filter(Boolean).join(' • ')||'Brak danych NOAA';els.kp.textContent=space.kp==null?'—':space.kp.toFixed(1);els.bz.textContent=space.bz==null?'—':space.bz.toFixed(1);els.kp.dataset.value=space.kp??'';els.bz.dataset.value=space.bz??'';els.status.textContent='Brak prognozy pogody';els.verdict.textContent='Nie znamy teraz zachmurzenia';els.summary.textContent=`Aktywność zorzowa jest ${activityLabel(act).toLowerCase()}, ale bez danych o chmurach nie policzymy szansy. Spróbuj ponownie za chwilę.`;els.tonightCard.hidden=true}
